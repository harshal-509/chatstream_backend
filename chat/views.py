...
def init(request):
    ...
    try:
        ...
        token = bytes(client.create_token(user_id=member.username),encoding='utf8').decode('utf-8')
        return JsonResponse(status=200, data={"username": member.username, "token": token, "apiKey": settings.STREAM_API_KEY})

    except Member.DoesNotExist:
        member = Member(username=username)
        member.save()
        token = bytes(client.create_token(user_id=username),encoding='utf8').decode('utf-8')
